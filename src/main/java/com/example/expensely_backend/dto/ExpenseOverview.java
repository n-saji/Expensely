...
public class ExpenseOverview {
	...
	@Getter
	private final List<RecurringExpenseDTO> upcomingRecurringExpenses;
	@Getter
	...
	public ExpenseOverview(List<ExpenseResponse> expenses, List<ExpenseResponse> req_expenses_range, List<ExpenseResponse> req_expenses_range_monthly,
		...
		for (Budget budget : budgetServiceMap) {
			budget.setUser(null);
			budget.getCategory().setUser(null);
			this.budgetServiceMap.put(budget.getId().toString(), budget);
		}

		this.upcomingRecurringExpenses = recurringExpenses.stream().map(
						e -> {
				...
				.filter(RecurringExpenseDTO::isActive)
				.filter(expense -> expense.getNextOccurrence() != null && !expense.getNextOccurrence().isBefore(LocalDate.now()))
				.sorted(Comparator.comparing(RecurringExpenseDTO::getNextOccurrence))
